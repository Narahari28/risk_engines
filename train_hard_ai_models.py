...
def fit_model_and_test_state_3():
  ...
  for i in range(len(x_test)):
    ...
    for j in range(len(row)):
      ...
      isValidAttack = attackPhrase == "endattack" or ((int(x_test[i][int(attackPhrase.split()[0]) - 1]) >= 0) and (x_test[i][int(attackPhrase.split()[1]) - 1] <= 0))
      if(val >= max_prob and isValidAttack):
        max_prob = val
        ans = j
    y_pred.append(ans)
  print("State 3 Accuracy:", metrics.accuracy_score(y_test, y_pred)) # Gets ~29.5%, much better than 1/164
